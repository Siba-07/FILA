...
class sarsaAgent():
    ...
    def __init__(self):
        ...
        self.weights_T1 = np.zeros((3,19*15))
        self.weights_T2 = np.zeros((3,8*9*9))
        self.discount = 1.0
    ...
    def get_table_features(self, obs):
        ...
        state_vec = np.zeros(19*15)

        ind = int(math.floor((x+1.2)/0.1)*15) + int(math.floor((v+0.07)/0.01))
    ...
    def get_better_features(self, obs):
        ...
        for i in range(8):
            indi = int(math.floor((x-hmin)/hside))*9 + int(math.floor((v-vmin)/vside))
            ind.append(81*i+indi) 
            hmin += hside/8
            vmin += vside/8
    ...
    def choose_action(self, state, weights, epsilon):
        ...
        if(sample<epsilon):
            ...
            val = []
            for i in range(3):
                val.append(np.sum(weights[i][state]))
            a = np.argmax(np.array(val))
        return a
    ...
    def sarsa_update(self, state, action, reward, new_state, new_action, learning_rate, weights):
        q_old = np.sum(weights[action][state])
        q_new = np.sum(weights[new_action][new_state])

        upd = learning_rate*(reward + self.discount*q_new - q_old)
        # print(weights[action][state],weights[new_action][new_state])
        # print(state,new_state)
        # print(upd,reward,q_old,q_new)
        # print(np.argmax(state))
        for st in state:
            weights[action][st] += upd 
